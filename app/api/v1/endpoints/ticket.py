...
async def create_ticket(
    ...
    ticket = await ticket_service.create_ticket(ticket_data, str(current_user.id))
    return ticket
...
async def update_ticket(
    ...
    ticket_service = TicketService(db)
    ticket = await ticket_service.update_ticket(ticket_id, ticket_data, str(current_user.id))
    if not ticket:
        raise HTTPException(status_code=404, detail="Ticket not found")
    return ticket
...
async def delete_ticket(
    ...
    ticket_service = TicketService(db)
    if not ticket_service.delete_ticket(ticket_id, str(current_user.id)):
        raise HTTPException(status_code=404, detail="Ticket not found")
...
async def list_tickets(
    ...
    """List tickets with filters"""
    # Pass the db session to check_permission
    if not check_permission(current_user, "ticket:read", db):
...
async def add_comment(
    ...
    ticket_service = TicketService(db)
    comment = await ticket_service.add_comment(ticket_id, comment_data, str(current_user.id))
    if not comment:
        raise HTTPException(status_code=404, detail="Ticket not found")
    return comment
...
async def update_comment(
    ...
    check_permission(current_user, "ticket:comment")
    ticket_service = TicketService(db)
    comment = ticket_service.update_comment(comment_id, comment_data, str(current_user.id))
    if not comment:
...
async def delete_comment(
    ...
    check_permission(current_user, "ticket:comment")
    ticket_service = TicketService(db)
    if not ticket_service.delete_comment(comment_id, str(current_user.id)):
        raise HTTPException(status_code=404, detail="Comment not found")
